...
impl CafeScene {
    ...
    fn render_counter_activity(&mut self, tick: u64, activity: &SceneActivity) {
        let intensity = non_cpu_activity(activity);
        if intensity <= 0.05 {
            return;
        }

        // Keep non-CPU signals clustered near the counter. A wider particle
        // field would look lively, but it would dirty too many Kitty tiles.
        let counter_top = cafe_counter_top(self.canvas.height());
        let center_x = self.canvas.width() / 2;
        let cup_x = center_x.saturating_add(self.canvas.width() / 12);
        let cup_y = counter_top.saturating_add(12);
        let steam_puffs = 2 + (intensity * 4.0).round() as u16;
        let drift =
            ((activity.network_download() + activity.network_upload()) * 8.0).round() as i16;
        ...
        for bubble in 0..bubbles.min(5) {
            ...
            draw_rect(&mut self.canvas, x, y, 2, 2, Rgba::rgb(235, 178, 89));
        }
